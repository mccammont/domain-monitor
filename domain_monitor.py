...
_rdap_bootstrap: Optional[Dict] = None
...
def get_whois_fallback(domain: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "creation_date": _safe_str(w.creation_date)[:10] if w.creation_date else None,
            "expiration_date": _safe_str(w.expiration_date)[:10] if w.expiration_date else None,
            "updated_date": _safe_str(w.updated_date)[:10] if w.updated_date else None,
            "name_servers": sorted([ns.lower() for ns in w.name_servers]) if w.name_servers else [],
        }
    except ImportError:
...
def get_ssl_info(domain: str, port: int = 443) -> Dict[str, Any]:
    ...
    try:
        ...
        not_before = datetime.strptime(not_before_raw, "%b %d %H:%M:%S %Y %Z").replace(tzinfo=timezone.utc)
    ...
    # Subject and issuer
    def _extract(field_list: List, key: str) -> Optional[str]:
    ...
    return {
        "subject_cn": _extract(subject, "commonName"),
        "issuer_o": _extract(issuer, "organizationName"),
        "issuer_cn": _extract(issuer, "commonName"),
    ...
    if expiry_str:
        try:
            ...
            if 0 <= days <= warn_days:
                ...
                warnings.append(
                    f"🔴  Domain {domain} appears to have EXPIRED {abs(days)} day(s) ago ({expiry_str})"
                )
        except ValueError:
            pass
...
def build_report(domain: str, changes: List[Tuple], warnings: List[str]) -> Tuple[str, str]:
    ...
    body = f"Domain Monitor report for {domain}\nRun at: {datetime.now().isoformat()}\n\n"
    body += "\n\n".join(parts)
...
""",
    ...
    parser.add_argument("--notify-email", metavar="EMAIL", help="Send email notification to this address")
    parser.add_argument(
        "--notify-macos",
        action="store_true",
    ...
    parser.add_argument(
        "--state-dir",
        default=None,
        help="Directory to store per-domain state files (default: platform-specific user data directory)",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    if not scope:
        log.error("No valid scope specified.")
        return 1
